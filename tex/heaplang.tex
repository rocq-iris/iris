...
All of this lets us define $\ofval$ as simply applying the value injection (the very first syntactic form of $\Expr$), which makes a lot of things in Rocq much simpler.
...
This lets us define the primitive reduction relation in terms of a ``primitive step'' reduction; see \figref{fig:heaplang-reduction-pure} and \figref{fig:heaplang-reduction-impure}.
...
\begin{figure}[p]
\judgment[Per-thread reduction]{\expr_1, \pstate_1 \step [\vec\obs] \expr_2, \pstate_2, \vec\expr}
\begin{mathpar}
\infer
  {\expr_1, \pstate_1 \hstep [\vec\obs] \expr_2, \pstate_2, \vec\expr}
  {\fillctx\lctx[\expr_1], \pstate_1 \step[\vec\obs] \fillctx\lctx[\expr_2], \pstate_2, \vec\expr}
\end{mathpar}

\judgment[``Primitive'' reduction (pure)]{\expr_1, \pstate_1 \hstep [\vec\obs] \expr_2, \pstate_2, \vec\expr}
\newcommand\alignheader{\kern-30ex}
\begin{align*}
&\alignheader\textbf{``Boxing'' reductions} \\
(\RecE\lvarF(\lvar)= \expr, \pstate) \hstep[\nil]{}&
  (\RecV\lvarF(\lvar)= \expr, \pstate, \nil) \\
((\val_1, \val_2)_\exprForm, \pstate) \hstep[\nil]{}&
  ((\val_1, \val_2)_\valForm, \pstate, \nil) \\
(\InlE(\val), \pstate) \hstep[\nil]{}&
  (\InlV(\val), \pstate, \nil) \\
(\InrE(\val), \pstate) \hstep[\nil]{}&
  (\InrV(\val), \pstate, \nil) \\
&\alignheader\textbf{Pure reductions} \\
((\RecV\lvarF(\lvar)= \expr)(\val), \pstate) \hstep[\nil]{}&
  (\subst {\subst \expr \lvarF {(\Rec\lvarF(\lvar)= \expr)}} \lvar \val, \pstate, \nil) \\
(-_{\HLOp} z, \pstate) \hstep[\nil]{}&
  (-z, \pstate, \nil) \\
(z_1 +_{\HLOp} z_2, \pstate) \hstep[\nil]{}&
  (z_1 + z_2, \pstate, \nil) \\
(z_1 -_{\HLOp} z_2, \pstate) \hstep[\nil]{}&
  (z_1 - z_2, \pstate, \nil) \\
(\loc \Ptradd z, \pstate) \hstep[\nil]{}&
  (\loc + z, \pstate, \nil) \\
(\val_1 =_{\HLOp} \val_2, \pstate) \hstep[\nil]{}&
  (\True, \pstate, \nil)
  &&\text{if $\val_1 \valeq \val_2$} \\
(\val_1 =_{\HLOp} \val_2, \pstate) \hstep[\nil]{}&
  (\False, \pstate, \nil)
  &&\text{if $\val_1 \valne \val_2$} \\
(\If \True then \expr_1 \Else \expr_2, \pstate) \hstep[\nil]{}&
  (\expr_1, \pstate, \nil) \\
(\If \False then \expr_1 \Else \expr_2, \pstate) \hstep[\nil]{}&
  (\expr_2, \pstate, \nil) \\
(\Fst((\val_1, \val_2)_\valForm), \pstate) \hstep[\nil]{}&
  (\val_1, \pstate, \nil) \\
(\Snd((\val_1, \val_2)_\valForm), \pstate) \hstep[\nil]{}&
  (\val_2, \pstate, \nil) \\
(\Match \InlV(\val) with \Inl => \expr_1 | \Inr => \expr_2 end, \pstate) \hstep[\nil]{}&
  (\expr_1(\val), \pstate, \nil) \\
(\Match \InrV(\val) with \Inl => \expr_1 | \Inr => \expr_2 end, \pstate) \hstep[\nil]{}&
  (\expr_2(\val), \pstate, \nil)
\end{align*}
...
\begin{figure}
\judgment[``Primitive'' reduction (impure)]{\expr_1, \pstate_1 \hstep [\vec\obs] \expr_2, \pstate_2, \vec\expr}
\newcommand\alignheader{\kern-30ex}
\begin{align*}
&\alignheader\textbf{Heap reductions} \\
(\AllocN(z, \val), \pstate) \hstep[\nil]{}&
  (\loc, \mapinsert {[\loc,\loc+z)} \val {\pstate:\stateHeap}, \nil)
  &&\text{if $z>0$ and \(\All i<z. \pstate.\stateHeap(\loc+i) = \bot\)} \\
(\Free(\loc), \pstate) \hstep[\nil]{}&
  (\TT, \mapinsert \loc \bot {\pstate:\stateHeap}, \nil) &&\text{if $\pstate.\stateHeap(l) = \val$} \\
(\deref\loc, \pstate) \hstep[\nil]{}&
  (\val, \pstate, \nil) &&\text{if $\pstate.\stateHeap(l) = \val$} \\
(\loc\gets\valB, \pstate) \hstep[\nil]{}&
  (\TT, \mapinsert \loc \valB {\pstate:\stateHeap}, \nil)  &&\text{if $\pstate.\stateHeap(l) = \val$} \\
(\CmpXchg(\loc,\valB_1,\valB_2), \pstate) \hstep[\nil]{}&
  ((\val, \True), \mapinsert\loc{\valB_2}{\pstate:\stateHeap}, \nil)
  &&\text{if $\pstate.\stateHeap(l) = \val$ and $\val \valeq \valB_1$} \\
(\CmpXchg(\loc,\valB_1,\valB_2), \pstate) \hstep[\nil]{}&
  ((\val, \False), \pstate, \nil)
  &&\text{if $\pstate.\stateHeap(l) = \val$ and $\val \valne \valB_1$} \\
(\Xchg(\loc, \valB), \pstate) \hstep[\nil]{}&
  (\val, \mapinsert \loc \valB {\pstate:\stateHeap}, \nil) &&\text{if $\pstate.\stateHeap(l) = \val$} \\
(\FAA(\loc, z_2), \pstate) \hstep[\nil]{}&
  (z_1, \mapinsert \loc {z_1+z_2} {\pstate:\stateHeap}, \nil) &&\text{if $\pstate.\stateHeap(l) = z_1$} \\
&\alignheader\textbf{Special reductions} \\
(\Fork\expr, \pstate) \hstep[\nil]{}&
  (\TT, \pstate, \expr) \\
(\NewProph, \pstate) \hstep[\nil]{}&
  (\prophid, \pstate:\stateProphs \uplus \set{\prophid}, \nil)
  &&\text{if $\prophid \notin \pstate.\stateProphs$}
\end{align*}
\begin{mathpar}
\infer
  {(\expr, \pstate) \hstep[\vec\obs] (\val, \pstate', \vec\expr\,')}
  {(\ResolveWith \expr at \prophid to \valB, \pstate) \hstep[\vec\obs \dplus {[(\prophid, (\val, \valB))]}] (\val, \pstate', \vec\expr\,')}\qquad\text{ if $\prophid \in \pstate.\stateProphs$}
\end{mathpar}
\caption{HeapLang impure reduction rules. \\ \small
Here, $\pstate:\stateHeap$ denotes $\sigma$ with the $\stateHeap$ field updated as indicated.
