...
(\Free(\loc), \pstate) \hstep[\nil]{}&
  ((), \mapinsert \loc \bot {\pstate:\stateHeap}, \nil) &&\text{if $\pstate.\stateHeap(l) = \val$} \\
...
\infer
  ...
  {(\ResolveWith \expr at \prophid to \valB, \pstate) \hstep[\vec\obs \dplus [(\prophid, (\val, \valB))]] (\val, \pstate', \vec\expr')}\text{ if $\prophid \in \pstate.\stateProphs$}
\end{mathpar}
