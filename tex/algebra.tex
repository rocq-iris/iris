...
In other words, as $n$ increases, $\dist{n}$ becomes more and more refined (\ruleref{ofe-mono})---and in the limit, it agrees with plain equality (\ruleref{ofe-limit}).
...
\begin{align*}
  ...
  X \subseteqN{n} Y \eqdef{}& \All m \leq n. m \in X \Ra m \in Y
\end{align*}
...
then there exists a unique\footnote{Uniqueness is not proven in Rocq.} COFE $\ofe$ such that $G(\ofe^{\textrm{op}}, \ofe) \cong \ofe$ (\ie the two are isomorphic in $\COFEs$).
\end{thm}
...
Think of \texttt{Prop} in Rocq or $\mathbb{B}$ in classical mathematics.
...
Notice that the existential quantification in this axiom is \emph{constructive}, \ie it is a sigma type in Rocq.
The purpose of this axiom is to compute $\melt_1$, $\melt_2$ completing the following square:

% RJ FIXME: Needs some magic to fix the baseline of the $\dist{n}$, or so
\begin{center}
\begin{tikzpicture}[every edge/.style={draw=none}]
  ...
  \path (a) edge node {$\dist{n}$} (b);
  \path (a12) edge node {$\dist{n}$} (b12);
