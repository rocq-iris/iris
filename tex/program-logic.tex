...
\footnote{Actually, in the Rocq development masks are restricted to a class of sets of natural numbers that contains all finite sets and is closed under union, intersection, difference and complement.
...
It should be monotone with respect to the step counter: $\stateinterp(\pstate, n_s, \vec\obs, n_t) \vs[\emptyset] \stateinterp(\pstate, n_s + 1, \vec\obs, n_t)$.
...
\begin{align*}
  \textdom{wp}(\stateinterp, \pred_F, \stuckness) \eqdef{}& \MU \textdom{wp\any rec}. \Lam \mask, \expr, \pred. \\
        ...
        & \Bigl(\toval(\expr) = \bot \land \All \pstate, n_s, \vec\obs, \vec\obs', n_t. \stateinterp(\pstate, n_s, \vec\obs \dplus \vec\obs', n_t) \vsW[\mask][\emptyset] {}\\
        &\qquad (s = \NotStuck \Ra \red(\expr, \pstate)) * \All \expr', \pstate', \vec\expr. (\expr, \pstate \step[\vec\obs] \expr', \pstate', \vec\expr) \wand \laterCredit{(n_\rhd(n_s)+1)} \wand {}\\
        &\qquad\qquad (\pvs[\emptyset]\later\pvs[\emptyset])^{n_\rhd(n_s)+1} \pvs[\emptyset][\mask]\stateinterp(\pstate', n_s + 1, \vec\obs', n + |\vec\expr|) * \textdom{wp\any rec}(\mask, \expr', \pred) * {}\\
...
{\toval(\expr) = \bot \and \mask_2 \subseteq \mask_1}
{{ {\begin{inbox}
~~(\All \pstate, n_s, \vec\obs, n_t. \stateinterp(\pstate, n_s, \vec\obs, n_t) \vsW[\mask_1, \emptyset] n \leq n_\rhd(n_s) + 1) \land {}\\
...
\begin{mathpar}
  ...
  { {\begin{inbox} % for some crazy reason, LaTeX is actually sensitive to the space between the "{ {" here and the "} }" below...
        ~~\All \pstate_1,\vec\obs,\vec\obs',n. \stateinterp(\pstate_1,n_s,\vec\obs \dplus \vec\obs', n_t) \vsW[\mask][\emptyset] (\stuckness = \NotStuck \Ra \red(\expr_1,\pstate_1)) * {}\\
        \qquad~ \All \expr_2, \pstate_2, \vec\expr.  (\expr_1, \pstate_1 \step[\vec\obs] \expr_2, \pstate_2, \vec\expr) \wand \laterCredit{(n_\rhd(n_s) + 1)} \wand (\pvs[\emptyset]\later\pvs[\emptyset])^{n_\rhd(n_s)}\pvs[\emptyset][\mask] {}\\
        \qquad\qquad\left(\stateinterp(\pstate_2,n_s+1,\vec\obs',n_t+|\vec\expr|) * \wpre[\stateinterp;\pred_F]{\expr_2}[\stuckness;\mask]{\Ret\var.\prop} * \Sep_{\expr_\f \in \vec\expr} \wpre[\stateinterp\pred_F]{\expr_\f}[\stuckness;\top]{\pred_F}\right)  {}\\
...
\begin{thm}[Adequacy]
  Assume we are given some $\vec\expr_1$, $\pstate_1$, $\vec\obs$, $\tpool_2$, $\pstate_2$ such that $(\vec\expr_1, \pstate_1) \tpsteps[\vec\obs] (\tpool_2, \pstate_2)$.
...
\begin{align*}
 &\TRUE \proves \pvs[\top] \Exists \stateinterp, \vec\pred, \pred_F. \stateinterp(\pstate_1,0,\vec\obs,0) * \left(\Sep_{\expr,\pred \in \vec\expr_1,\vec\pred} \wpre[\stateinterp;\pred_F]{\expr}[\stuckness;\top]{x.\; \pred(x)}\right) * \left(\consstate^{\stateinterp;\vec\pred;\pred_F}_{\stuckness}(\tpool_2, \pstate_2) \vs[\top][\emptyset] \hat{\metaprop}\right)
\end{align*}
where $\consstate$ describes states that are consistent with the state interpretation and postconditions:
\begin{align*}
 \consstate^{\stateinterp;\vec\pred;\pred_F}_{\stuckness}(\tpool_2, \pstate_2) \eqdef{}&\Exists \vec\expr_2, \tpool_2'. \tpool_2 = \vec\expr_2 \dplus \tpool_2' * {}\\
 &\quad |\vec\expr_1| = |\vec\expr_2| *{}\\
 &\quad (s = \NotStuck \Ra \All \expr \in \tpool_2. \toval(\expr) \neq \bot \lor \red(\expr, \pstate_2) ) *{}\\
 &\quad \stateinterp(\pstate_2, (), |\tpool_2'|) *{}\\
...
\begin{itemize}
  ...
  \item and a view shift showing the desired $\hat\metaprop$ under the extra assumption $\consstate(\tpool_2, \pstate_2)$.
\end{itemize}
...
$\consstate^{\stateinterp;\vec\pred;\pred_F}_{\stuckness}(\tpool_2, \pstate_2)$ says that:
\begin{itemize}
...
\item If this is a stuck-free weakest precondition, then all threads in the final thread-pool are either values or are reducible in the final state $\pstate_2$.
...
\[
\TRUE \proves \All\pstate_1, \vec\obs. \pvs[\top] \Exists \stateinterp, \pred, \pred_F. \stateinterp(\pstate_1,0,\vec\obs,0) * \wpre[\stateinterp;\pred_F]{\expr_1}[\NotStuck;\top]{x.\; \pred(x)}
\]
  Then it is the case that:
\[
\All \pstate_1, \vec\obs, \tpool_2, \pstate_2. ([\expr_1], \pstate_1) \tpsteps[\vec\obs] (\tpool_2, \pstate_2) \Ra \All \expr \in \tpool_2. \toval(\expr) \neq \bot \lor \red(\expr, \pstate_2)
\]
\end{cor}
To prove the conclusion of this corollary, we assume some $\pstate_1, \vec\obs, \tpool_2, \pstate_2$ and $([\expr_1], \pstate_1) \tpsteps[\vec\obs] (\tpool_2, \pstate_2)$, and we instantiate the main theorem with this execution and $\metaprop \eqdef \All \expr \in \tpool_2. \toval(\expr) \neq \bot \lor \red(\expr, \pstate_2)$.
We can then show the premise of adequacy using the Iris entailment that we assumed in the corollary and:
\[ \TRUE \proves \consstate^{\stateinterp;[\pred];\pred_F}_{\NotStuck}(\tpool_2, \pstate_2) \vs[\top][\emptyset] \metaprop \]
...
\[
\TRUE \proves \All\pstate_1, \vec\obs. \pvs[\top] \Exists \stateinterp, \pred_F. \stateinterp(\pstate_1,0,\vec\obs,0) * \wpre[\stateinterp;\pred_F]{\expr_1}[\stuckness;\top]{x.\; x \in V}
\]
  Then it is the case that:
\[
\All \pstate_1, \vec\obs, \val_2, \tpool_2, \pstate_2. ([\expr_1], \pstate_1) \tpsteps[\vec\obs] ([\ofval(\val_2)] \dplus \tpool_2, \pstate_2) \Ra \val_2 \in V
\]
\end{cor}
To show this, we assume some $\pstate_1, \vec\obs, \val_2, \tpool_2, \pstate_2$ such that $([\expr_1], \pstate_1) \tpsteps[\vec\obs] ([\ofval(\val_2)] \dplus \tpool_2, \pstate_2)$, and we instantiate adequacy with this execution and $\metaprop \eqdef \val_2 \in \Val$.
Then we only have to show:
$$\TRUE \proves \consstate^{\stateinterp;[(\Lam \val. \val \in \Val)];\pred_F}_{\stuckness}([\ofval(\val_2)] \dplus \tpool_2, \pstate_2) \vs[\top][\emptyset] \val_2 \in \Val $$

~\par

As a final example, we could use adequacy to show that the state $\pstate$ of the program is always in some set $\Sigma \subseteq \State$:
...
\[
\TRUE \proves \All\pstate_1, \vec\obs. \pvs[\top] \Exists \stateinterp, \pred, \pred_F. \stateinterp(\pstate_1,0,\vec\obs,0) * \wpre[\stateinterp;\pred_F]{\expr_1}[\stuckness;\top]{\pred} * (\All \pstate_2, n_s, n_t. \stateinterp(\pstate_2,n_s,(),n_t) \!\vs[\top][\emptyset] \pstate_2 \in \Sigma)
\]
  Then it is the case that:
\[
\All \pstate_1, \vec\obs, \tpool_2, \pstate_2. ([\expr_1], \pstate_1) \tpsteps[\vec\obs] (\tpool_2, \pstate_2) \Ra \pstate_2 \in \Sigma
\]
\end{cor}
To show this, we assume some $\pstate_1, \vec\obs, \tpool_2, \pstate_2$ such that $([\expr_1], \pstate_1) \tpsteps[\vec\obs] (\tpool_2, \pstate_2)$, and we instantiate adequacy with this execution and $\metaprop \eqdef \pstate_2 \in \Sigma$.
Then we have to show:
\[
(\All \pstate_2, n_s, n_t. \stateinterp(\pstate_2,n_s,(),n_t) \!\vs[\top][\emptyset] \pstate_2 \in \Sigma) \proves \consstate^{\stateinterp;[\pred];\pred_F}_{\stuckness}(\tpool_2, \pstate_2) \vs[\top][\emptyset] \pstate_2 \in \Sigma
\]

\paragraph{Hoare triples.}
It turns out that weakest precondition is actually quite convenient to work with, in particular when performing these proofs in Rocq.
...
(In other words, the list is ``backwards''. This is because cons-ing to the list, like the dot does above, is easier to deal with in Rocq than appending at the end.)
