...
\end{align*}
Note that in the definition of the carrier $\latert\cofe$, $\latertinj$ is a constructor (like the constructors in Rocq), \ie this is short for $\setComp{\latertinj(x)}{x \in \cofe}$.
...
{\All n, \melt, \meltB. \melt \mincl[n] \meltB \Ra \pred(\melt) \subseteqN{n} \pred(\meltB)} \\
...
Note that we will not explicitly deal with equivalence classes below but instead just pick an arbitrary element of the class.
There is an implicit proof obligation for every operation acting on $\UPred$ to show that the concrete representative does not matter.
This is all explicit in Rocq (where we use a setoid to represent the quotient); we do not spell it out on paper.
...
\begin{align*}
  \All n, \melt.  (\mval(\melt) \subseteqN{n} P_0(\melt)) \Ra n \in P_0(\melt)  \tagH{UPred-canonical}
\end{align*}
...
\begin{mathpar}
  \infer{x \dist{n} y}{\cinl(x) \dist{n} \cinl(y)}

  \infer{x \dist{n} y}{\cinr(x) \dist{n} \cinr(y)}

  \axiom{\mundef \dist{n} \mundef}
\end{mathpar}
...
~\\
%    \All n \in {\melt.V}.\, \melt.x \dist{n} \meltB.x \\
...
\begin{mathpar}
  ...
  \axiomH{ag-agree}{n \in \mval(\aginj(x) \mtimes \aginj(y)) \Ra x \dist{n} y}
\end{mathpar}
...
The step-indexed equivalence is inductively defined as follows:
\begin{mathpar}
  \infer{x \dist{n} y}{\exinj(x) \dist{n} \exinj(y)}
...
%TODO: These need syncing with Rocq
