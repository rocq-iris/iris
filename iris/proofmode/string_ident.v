...
Module StringToIdent.
  ...
  Ltac2 rocq_byte_to_int (b : constr) : int :=
    match! b with
  ...
  Ltac2 rocq_byte_to_char (b : constr) : char :=
    Char.of_int (rocq_byte_to_int b).

  Fixpoint rocq_string_to_list_byte (s : string) : list byte :=
    match s with
    | EmptyString => []
    | String c s => Ascii.byte_of_ascii c :: rocq_string_to_list_byte s
    end.

  (** copy a list of Rocq byte constrs into a string (already of the right length) *)
  Ltac2 rec rocq_byte_list_blit_list (pos : int) (ls : constr) (str : string) : unit :=
    match! ls with
    | nil => ()
    | ?c :: ?ls =>
      let b := rocq_byte_to_char c in
      String.set str pos b; rocq_byte_list_blit_list (Int.add pos 1) ls str
    end.

  Ltac2 rec rocq_string_length (s : constr) : int :=
    match! s with
    | EmptyString => 0
    | String _ ?s' => Int.add 1 (rocq_string_length s')
  ...
  (** [rocq_string_to_string] converts a Gallina string in a constr to an Ltac2
  native string *)
  Ltac2 rocq_string_to_string (s : constr) : string :=
    let l := rocq_string_length s in
    let str := String.make l (Char.of_int 0) in
    let bytes := compute constr:(rocq_string_to_list_byte $s) in
    let _ := rocq_byte_list_blit_list 0 bytes str in
    str.
  ...
  (** [rocq_string_to_ident] implements the ident to string conversion in Ltac2 *)
  Ltac2 rocq_string_to_ident (s : constr) : ident :=
    string_to_ident (rocq_string_to_string s).

  (** We want to wrap [rocq_string_to_ident] in an Ltac1 API, but Ltac1-2 FFI
  ...
  Ltac string_to_ident_cps :=
    ltac2:(s1 r |- let s := Option.get (Ltac1.to_constr s1) in
                   let ident := rocq_string_to_ident s in
