...
Local Instance bi_rtc_pre_mono {SI : sidx} {PROP : bi} `{!Sbi PROP}
...
Global Instance bi_rtc_ne {SI : sidx} `{!Sbi PROP} {A : ofe} (R : A → A → PROP) :
...
Global Instance bi_rtc_proper {SI : sidx} `{!Sbi PROP} {A : ofe} (R : A → A → PROP)
  : Proper ((≡) ==> (≡) ==> (⊣⊢)) (bi_rtc R).
Proof. apply ne_proper_2. apply _. Qed.

Local Instance bi_tc_pre_mono {SI : sidx} `{!Sbi PROP}
    {A : ofe} (R : A → A → PROP) `{NonExpansive2 R} (x : A) :
...
Global Instance bi_tc_ne {SI : sidx} `{!Sbi PROP}
    {A : ofe} (R : A → A → PROP) `{NonExpansive2 R} :
...
Global Instance bi_tc_proper {SI : sidx} `{!Sbi PROP}
    {A : ofe} (R : A → A → PROP) `{NonExpansive2 R}
  : Proper ((≡) ==> (≡) ==> (⊣⊢)) (bi_tc R).
Proof. apply ne_proper_2. apply _. Qed.

Global Instance bi_nsteps_ne {SI : sidx} `{!Sbi PROP}
    {A : ofe} (R : A → A → PROP) `{NonExpansive2 R} (n : nat) :
  NonExpansive2 (bi_nsteps R n).
Proof. induction n; solve_proper. Qed.

Global Instance bi_nsteps_proper {SI : sidx} `{!Sbi PROP}
    {A : ofe} (R : A → A → PROP) `{NonExpansive2 R} (n : nat)
...
Section timeless.
  Context {SI : sidx} `{!Sbi PROP, !Timeless (PROP:=PROP) emp}.
