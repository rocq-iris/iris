...
Section sep_list.
  ...
  Global Instance big_sepL_id_mono' :
    Proper (Forall2 (⊢) ==> (⊢)) (big_opL (@bi_sep _ PROP) (λ _ P, P)).
...
Lemma big_sepM_set_to_map `{Countable K} {A}
    (Φ : K → A → PROP) (X : gset K) (g : K → A) :
  ([∗ map] k↦a ∈ set_to_map g X, Φ k a) ⊣⊢ ([∗ set] k ∈ X, Φ k (g k)).
Proof. apply big_opM_set_to_map. Qed.
